<script setup></script>

<template>
  <div class="container d-flex justify-content-center align-items-center vh-100 flex-column">
    <div class="d-flex align-items-center justify-content-center">
      <div class="card p-4" style="font-size: 20px;">
        <p class="text-center" style="font-size: larger;"><strong>Quais categorias você se identifica?</strong></p>
        <div class="row">
          <div class="col-md-6">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria1" />
              <label class="form-check-label" for="categoria1">
                Eletrodomésticos
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria2" />
              <label class="form-check-label" for="categoria2">
                Nutrição
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria3" />
              <label class="form-check-label" for="categoria3">
                Eletrônicos
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria4" />
              <label class="form-check-label" for="categoria4"> Moda </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria5" />
              <label class="form-check-label" for="categoria5">
                Beleza e Cuidados Pessoais
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria6" />
              <label class="form-check-label" for="categoria6">
                Livros e Mídia
              </label>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria7" />
              <label class="form-check-label" for="categoria7">
                Casa e Jardim
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria8" />
              <label class="form-check-label" for="categoria8">
                Esportes e Lazer
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria9" />
              <label class="form-check-label" for="categoria9">
                Automotivo
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria10" />
              <label class="form-check-label" for="categoria10">
                Brinquedos e Jogos
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria11" />
              <label class="form-check-label" for="categoria11">
                Alimentos e Bebidas
              </label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="checkbox" value="" id="categoria12" />
              <label class="form-check-label" for="categoria12">
                Tecnologia
              </label>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
